...
class BaseContainerEdit(BaseEdit):
    ...
    def _dict_insert(
        ...
        if not isinstance(dict_, OrderedDict):
            self._dict_add(key, value_tuple[1], dict_, inverse_diff_dict)
    ...
    def _dict_add_or_modify(self, key, value, dict_, inverse_diff_dict=None):
        ...
        """
        if key in dict_:
            self._dict_modify(key, value, dict_, inverse_diff_dict)
        else:
            self._dict_add(key, value, dict_, inverse_diff_dict)
    ...
    def _dict_remove_or_modify(
        ...
        """
        if value is None:
            self._dict_remove(key, value, dict_, inverse_diff_dict)
        else:
            self._dict_modify(key, value, dict_, inverse_diff_dict)
    ...
    def _list_remove(self, index, list_, inverse_diff_list=None):
        """Remove item at given index from list.
...
class TimelineEdit(CompositeEdit):
    ...
    def __init__(
        ...
        """
        if op_type in (ContainerOp.ADD, ContainerOp.INSERT):
            add_edit = BaseContainerEdit(
                timeline._dict,
                diff_dict,
                op_type,
                recursive=True,
            ...
            super(TimelineEdit, self).__init__([add_edit], register_edit)

        elif op_type == ContainerOp.REMOVE:
            # convert diff list to use indexes instead of items
            remove_edit = BaseContainerEdit(
                timeline._dict,
                OrderedDict([
                    (time, [item_list.index(item) for item in item_list])
            ...
            super(TimelineEdit, self).__init__([remove_edit], register_edit)
